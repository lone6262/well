...
Page({
  ...
  parseContent: function (content) {
    ...
    let html = content
      ...
      .replace(
        /^# (.+)$/gm,
        '<h1 style="font-size:24px;font-weight:bold;margin:16px 0 8px;color:#2D2A26;">$1</h1>'
      )
      // 二级标题
      .replace(
        /^## (.+)$/gm,
        '<h2 style="font-size:20px;font-weight:bold;margin:14px 0 6px;color:#2D2A26;">$1</h2>'
      )
      // 三级标题
      .replace(
        /^### (.+)$/gm,
        '<h3 style="font-size:18px;font-weight:bold;margin:12px 0 6px;color:#2D2A26;">$1</h3>'
      )
      // 引用块
      .replace(
        /^> (.+)$/gm,
        '<div style="background:#FEF7E0;padding:12px;border-left:4px solid #F5D547;margin:12px 0;border-radius:4px;color:#666;">$1</div>'
      )
      // 列表项
      ...
      .replace(
        /^(?!<[hH]|[pP]|$)(.+)$/gm,
        '<p style="line-height:1.6;margin:12px 0;color:#4A4743;">$1</p>'
      );
