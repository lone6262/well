...
Page({
  ...
  loadAssessmentDetail: function () {
    ...
    // 设置会员状态
    self.setData({
      isMember: (app.globalData.userInfo && app.globalData.userInfo.isMember) || false,
    });
      ...
      success: function (res) {
        ...
        if (res.result.code === 0) {
          ...
          self.setData({
            ...
            selectedSymptoms:
              data.assessmentDetail.symptom_names || data.assessmentDetail.symptoms || [], // 优先使用symptom_names（中文名称）
        ...
        if (res.result && res.result.code === 0) {
          ...
          if (orders && orders.length > 0) {
            ...
            wx.showModal({
              title: '待支付订单',
              content:
                '您有一笔未完成的订单（' + (order.description || 'AI报告') + '），是否继续支付？',
              confirmText: '继续支付',
              cancelText: '取消订单',
