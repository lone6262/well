...
const loadCoupons = createCrudLoader({
  ...
  getState: function () {
    return { page: couponsCurrentPage };
  },
  setState: function (state) {
    couponsCurrentPage = state.page;
  },
  getFilters: function () {
    return {
  ...
  try {
    ...
    if (result.code === 0 && result.data) {
      ...
      document.getElementById('couponDiscountValue').value =
        c.discount_type === 'fixed' ? ((c.discount_value || 0) / 100).toFixed(2) : c.discount_value;
  ...
  const validation = validateAndShow(
    {
      ...
      couponDiscountValue: [
        { required: true, message: '请输入折扣值' },
        {
          number: true,
          min: 0.01,
          max: discountType === 'percent' ? 9.9 : undefined,
          message:
            discountType === 'percent'
              ? '折扣必须在 0.01 到 9.9 之间（8 = 8 折）'
              : '固定减金额必须大于 0',
        },
      ],
...
function initCouponsModule() {
  bindFilterEvents(
    [{ elementId: 'couponStatusFilter' }, { elementId: 'couponSceneFilter' }],
    loadCoupons
  );
