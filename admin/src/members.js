...
import { STATUS_MAP, MEMBER_TYPE_MAP } from './config.js';
import { callCloudFunction, showLoading, hideLoading, showToast, handleApiError } from './api.js';
import { formatDate, escapeHtml, escapeAttr } from './utils.js';
...
function displayMembers(members) {
  ...
  tbody.innerHTML = members.map(function(member) {
    var typeText = MEMBER_TYPE_MAP[member.type] || member.type || '-';
    var statusInfo = STATUS_MAP.member[member.status] || { text: member.status, class: 'badge-gray' };
    var expireDate = formatDate(member.expire_date);
    var isExpired = member.status !== 'active';
    var creditsUsed = member.report_credits_used || 0;
    var creditsTotal = member.report_credits_total || 0;
    var autoRenew = member.auto_renew ? '✅ 开启' : '❌ 关闭';

    return '<tr>' +
      '<td>' + escapeHtml(member.userInfo ? member.userInfo.nickName || (member.user_id || '').substring(0, 8) : (member.user_id || '').substring(0, 8) || '-') + '</td>' +
      ...
      '<td>' + autoRenew + '</td>' +
      '<td>' +
        '<button class="btn btn-sm btn-outline" onclick="window.__members.viewMemberDetail(\'' + escapeAttr(member._id) + '\')">详情</button>' +
        (!isExpired ? ' <button class="btn btn-sm btn-outline" onclick="window.__members.cancelMember(\'' + escapeAttr(member._id) + '\')">取消</button>' : '') +
      '</td>' +
    '</tr>';
  }).join('');
}
...
async function viewMemberDetail(memberId) {
  ...
  try {
    ...
    if (result.code === 0 && result.data) {
      ...
      if (m.family_member_ids && m.family_member_ids.length > 0) {
        info.push('家庭成员: ' + m.family_member_ids.length + ' 人');
      }
      if (m.auto_renew && m.renew_price) {
        info.push('续费价格: ¥' + ((m.renew_price || 0) / 100).toFixed(2));
      }
      alert(info.join('\n'));
    } else {
...
export function initMembersModule() {
  window.__members = { viewMemberDetail: viewMemberDetail, cancelMember: cancelMember };
  ...
  if (searchBtn) {
    searchBtn.addEventListener('click', function() { loadMembers(1); });
  }
